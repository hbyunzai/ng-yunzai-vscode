...
async function do_cache() {
  ...
  try {
    ...
    notifier.notify('zap', localize('again', 'NG-YUNZAI CSS classes cached (click to cache again), enter: {0}', cssRes.filePath));
  } catch (err) {
...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  emmetDisposables.push(
    commands.registerCommand('ng-yunzai-vscode.cache', async () => {
      if (CONFIG.caching) {
        return;
      }
